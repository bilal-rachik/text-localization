...
image_path = Path(r'C:\DEV\text-localization\data\train')
box_path = image_path.parent.joinpath('box')
...
def list():
    outfile = box_path.joinpath("jpglist.txt")
    wildcard = ".jpg"
...
# draw box
def draw():
    f = open(box_path.joinpath('jpglist.txt').as_posix())
    ...
    while line:
        ...
        img = cv2.imread(image_path.joinpath(name).as_posix())
        img_size = img.shape
        img_size = img_size[0]*img_size[1]

        # read each coordinate and draw box
        f_txt = open(image_path.joinpath(name.strip('.jpg') + '.txt').as_posix())
        ...
        while line_txt:
            ...
            line_txt = f_txt.readline()
        cv2.imwrite(box_path.joinpath(name).as_posix(), img)
        line = f.readline()
